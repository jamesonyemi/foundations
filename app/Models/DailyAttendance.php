...
class DailyAttendance extends Model
{
    ...
    {
        $fullday = count(DB::select(DB::raw("select * from daily_attendance where YEAR(date)=" . $year . "
	                                            AND MONTH(date)=" . $month . " AND daily_attendance.status='present'
	                                            AND employee_id='$employeeID'")));

        $halfday = count(DB::select(DB::raw("select * from daily_attendance where YEAR(date)=" . $year . "
    ...
    public static function attendanceCount($employeeID, $company_id)
    {
        ...
        $presentCount = DailyAttendance::countPresentDays($month, $year, $employeeID);
    ...
    public static function absentEmployee($employeeID)
    {
        ...
        $absent = [];
        foreach (Leavetype::get() as $leave) {
            $half_day = DailyAttendance::where('attendance.status', '=', 'absent')->where(function ($query) {
            ...
            // Added to casual
            $half = $half_day / 2;
            $absent[$leave->leaveType] = DailyAttendance::where('attendance.status', '=', 'absent')->where(function ($query) {
                $query->where('application_status', '=', 'approved')
                    ->orWhere('application_status', '=', null);
            })->where('employee_id', '=', $employeeID)//For Unpaid Leaves
    ...
    public static function absentEveryEmployee()
    {
        ...
        foreach ($employees as $employee) {
            ...
            foreach (Leavetype::get() as $leave) {
                ...
                $half_day = DailyAttendance::where('status', '=', 'absent')
                    ->where('employee_id', '=', $employee->id)
                    ->where(function ($query) {
                        $query->where('application_status', '=', 'approved')
                            ->orWhere('application_status', '=', null);
    ...
    public function getDdateAttribute($value)
    {
        if ($value == null) {
            return $value;
        }
        ...
        /*$carbon = Carbon::createFromFormat("Y-m-d 00:00:00", $this->attributes["date"]);*/
        $carbon = date("Y-m-d 00:00:00", strtotime($this->attributes["date"] ));;
    ...
    public function getDateAttribute($value)
    {
        ...
        /*$carbon = Carbon::createFromFormat("Y-m-d 00:00:00", $this->attributes["date"] . " 00:00:00");*/
        /*$carbon = Carbon::createFromFormat("Y-m-d 00:00:00", $this->attributes["date"]);*/
        $carbon = date("Y-m-d H:i:s", strtotime($this->attributes["date"] ));;
    ...
    public function getTimeAttribute($value)
    {
        ...
        /*$carbon = Carbon::createFromFormat("Y-m-d 00:00:00", $this->attributes["date"] . " 00:00:00");*/
        /*$carbon = Carbon::createFromFormat("Y-m-d 00:00:00", $this->attributes["date"]);*/
        $carbon = date("H:i:s", strtotime($this->attributes["date"] ));;
