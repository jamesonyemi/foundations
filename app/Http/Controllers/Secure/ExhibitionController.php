...
use App\Http\Requests\Secure\ExhibitionApplicantRequest;
...
use App\Notifications\ExhibitionRegisterNotification;
...
class ExhibitionController extends SecureController
{
    /**
    ...
    public function index()
    {
        ...
        $sections = ExhibitionApplicant::all();

        return view('exhibition.index', compact('title', 'sections'));
    }
    ...
    {
        $title = 'Welcome to Jospong Group';

        $interests = ExhibitionInterest::get();

        $countries = Country::get()
            ->pluck('name', 'id')
            ->prepend(trans('student.select_country'), '')
            ->toArray();
        return view('exhibition._form', compact('title', 'interests', 'countries'));
    }
     ...
     * @return Response
     */
    public function store(ExhibitionApplicantRequest $request)
    {
        try {
            DB::transaction(function () use ($request) {
                $exhibitionApplicant = ExhibitionApplicant::firstOrCreate
                (
                    [
                ...
                $exhibitionApplicant->interests()->attach($request->input('interest_id'));

                //Send email to responsible employees

                /*if (GeneralHelper::validateEmail($request->email_address))
                {
                    @Notification::send($exhibitionApplicant, new ExhibitionRegisterNotification($exhibitionApplicant));
                }*/
        ...
        return response('<div class="alert alert-success">Thank you, Information Saved Successfully</div>');
    }
