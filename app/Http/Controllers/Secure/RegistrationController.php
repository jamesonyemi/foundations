...
class RegistrationController extends SecureController
{
    ...
    public function index()
    {
        ...
        return view('registration.index', compact('title', 'schoolType', 'count', 'students'));
    }
    ...
    public function create()
    {
        ...
        $schoolyears = $this->schoolYearRepository
            ...
            ->get()
            ->pluck('title', 'id')
            ->prepend(trans('student.select_academic_year'), '')
            ->toArray();
        ...
        $semesters = $this->semesterRepository
            ...
            ->map(function ($item) {
                return [
                    'id'   => $item->id,
                    'title' => isset($item->school_year) ? $item->school_year->title.'  '.' |'.$item->title : '',
                ];
            })
            ->pluck('title', 'id')
        ...
        return view('layouts.create', compact('title', 'sections', 'directions', 'levels', 'subjects', 'sessions', 'graduationyears', 'semesters', 'schoolyears'));
    }
    ...
    public function store(RegistrationRequest $request)
    {
        ...
        foreach ($request['user_id'] as $user_id) {
            $student = Employee::where('user_id', $user_id)->first();
            ...
            if (! isset($user_exists->id)) {
                foreach ($request['subject_id'] as $subject_id) {
                    ...
                    $registration->company_year_id = session('current_company_year');
    ...
    public function store__(RegistrationRequest $request)
    {
        foreach ($request['user_id'] as $user_id) {
            $student = Employee::where('user_id', $user_id)->first();
            ...
            if (! isset($user_exists->id)) {
                foreach ($request['subject_id'] as $subject_id) {
                    $registration = new Registration($request->except('session_id', 'section_id', 'direction_id'));
                    $registration->user_id = $user_id;
                    $registration->student_id = $student->id;
                    $registration->subject_id = $subject_id;
                    $registration->company_id = session('current_company');
    ...
    public function update(RegistrationRequest $request, Registration $registration)
    {
        ...
        $student = Employee::where('id', $id)->first();
        if (isset($request->level_id) && ! empty($request->level_id)) {
            $student->level_id = $request['level_id'];
        }
    ...
    public function courses(Employee $registration)
    {
        $id = $registration->id;
    ...
    public function courses_data(Employee $registration)
    {
        $studentCourses = $this->registrationRepository->getAllForStudent($registration->user_id)
            ->get()
    ...
    public function get_courses(Employee $registration)
    {
