...
class StudentAlumniController extends SecureController
{
    /**
    ...
    {
        $title = trans('student.alumni');

        $maleStudents = $this->studentRepository
            ->getAllAlumniMale(session('current_company_year'), session('current_company_semester'), session('current_company'))
            ->get();

        $femaleStudents = $this->studentRepository
            ->getAllAlumniFemale(session('current_company_year'), session('current_company_semester'), session('current_company'))
            ->get();
        ...
        $students = $this->studentRepository->getAllAlumni(session('current_company_year'), session('current_company_semester'), session('current_company'))
            ->with('user', 'section', 'programme')
            ->orderBy('students.id')
            ->get();


        return view('alumni.index', compact('title', 'maleStudents', 'femaleStudents', 'sections', 'sessions', 'schoolyears', 'semesters', 'directions', 'levels', 'intakeperiods', 'entrymodes', 'campus', 'countries', 'countries2', 'maritalStatus', 'religions', 'students', 'graduationyears'));
    }
    ...
    {
        $title = trans('student.new_alumni');
        $this->generateParams();
        ...
        return view('layouts.create', compact('title', 'sections', 'sessions', 'directions', 'levels', 'intakeperiods', 'entrymodes', 'campus', 'countries', 'maritalStatus', 'religion', 'document_types', 'custom_fields', 'schoolYears'));
    }
    ...
    public function store(GraduatingRequest $request)
    {
        ...
        foreach ($request['student_id'] as $student_id) {
            Alumnus::firstOrCreate(['company_id' => session('current_company'),
                'school_year_id' => $request['graduation_year_id'],
        ...
        return redirect('/student/alumni')->with('status', 'Operation Successfully!');
    }
...
/*    public function export()
    ...
    {
        $students = $this->studentRepository->getAllAlumniFilter($request)->get();
