...
class StudentDropController extends SecureController
{
    /**
    ...
    {
        $title = trans('student.drop_student');

        $maleStudents = $this->studentRepository
            ->getAllDropMale(session('current_company_year'), session('current_company_semester'), session('current_company'))
            ->get();

        $femaleStudents = $this->studentRepository
            ->getAllDropFemale(session('current_company_year'), session('current_company_semester'), session('current_company'))
            ->get();
        ...
        $students = $this->studentRepository->getAllDrop(session('current_company_year'), session('current_company_semester'), session('current_company'))
            ->with('user', 'section', 'programme')
            ->get();


        return view('student_drop.index', compact('title', 'maleStudents', 'femaleStudents', 'sections', 'sessions', 'schoolyears', 'semesters', 'directions', 'levels', 'intakeperiods', 'entrymodes', 'campus', 'countries', 'countries2', 'maritalStatus', 'religions', 'students', 'graduationyears'));
    }
    ...
    public function store(GraduatingRequest $request)
    {
        ...
        foreach ($request['student_id'] as $student_id) {
            StudentDrop::firstOrCreate(['company_id' => session('current_company'),
        ...
        return redirect('/student/student_drop')->with('status', 'Student(s) Dropped Successfully!');
    }
...
/*    public function export()
    ...
    {
        $students = $this->studentRepository->getAllDropFilter($request)->get();
