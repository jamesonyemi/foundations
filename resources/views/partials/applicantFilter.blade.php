...
{{--<script type="text/javascript">
    $(document).ready(function(){
        ...
        var desdiv="direction_id";
        $(document.getElementById(srcdiv)).on('change',function(){
            ...
            $.ajax({
                type:'get',
                url:'{!!URL::to('admin/findProgName')!!}',
                data:{'id':section_id},
                success:function(data){


                    op+='<option value="0">Please Select Programme</option>';
                    for(var i=0;i<data.length;i++){

                        op+='<option value="'+data[i].id+'">'+data[i].name+'</option>';
                    }

                    div.find(document.getElementById(desdiv)).html("");
                    div.find(document.getElementById(desdiv)).append(op);
...
?>
         ...
         <div class="row">
            ...
            <select name="enroll" class="form-control select2" id="enroll" onChange="OpenAjaxPostCmd('{{ url('/ajax/'.$type) }}','result_container','appfilter','Please wait','result_container','1','2')">
            <option value="" selected="SELECTED">{{trans('student.select_status')}}</option>

            <option value="enroll">Enrolled</option>
            <option value="notenroll">Not Enrolled</option>
...
//    *********************FIND SELECTED SECTION PROGRAMS************************************
